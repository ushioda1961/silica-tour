    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#1a3a2a,#2d5a3a)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Hiragino Kaku Gothic ProN','Meiryo',sans-serif" }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: '40px 36px', width: '100%', maxWidth: 360, boxShadow: '0 8px 40px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="password" value={password} onChange={e => { setPassword(e.target.value); setPwError('') }} onKeyDown={e => e.key === 'Enter' && handleLogin()} placeholder="パスワードを入力"
  ...
  if (userInfo?.role === 'maker') return (
    <div style={{ fontFamily: "'Hiragino Kaku Gothic ProN','Meiryo',sans-serif", background: '#f0f4f8', minHeight: '100vh' }}>
      <div style={{ background: 'linear-gradient(135deg,#1a3a2a,#2d5a3a)', padding: '0 24px' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', padding: '14px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ fontSize: 20 }}>🏭</div>
            <div>
              <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)' }}>MAKER VIEW</div>
              <div style={{ fontSize: 14, fontWeight: 800, color: '#fff' }}>参加者サマリー（メーカー用）</div>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '24px 18px' }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: '24px', marginBottom: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          <div style={{ fontSize: 16, fontWeight: 900, color: '#1a3a2a', marginBottom: 20 }}>🎁 お土産準備用 参加者内訳</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              { label: '初回参加（代表者）', count: firstTimers, icon: '🆕', color: '#2d7a4a', bg: '#f0fdf4', border: '#86efac' },
              { label: '初回参加（同伴者）', count: firstTimerCompanions, icon: '🆕', color: '#2d7a4a', bg: '#f0fdf4', border: '#86efac' },
              { label: 'リピート（代表者）', count: repeaters, icon: '↩️', color: '#6366f1', bg: '#eef2ff', border: '#a5b4fc' },
              { label: 'リピート（同伴者）', count: repeaterCompanions, icon: '↩️', color: '#6366f1', bg: '#eef2ff', border: '#a5b4fc' },
            ].map(s => (
              <div key={s.label} style={{ background: s.bg, border: `1.5px solid ${s.border}`, borderRadius: 12, padding: '16px', textAlign: 'center' }}>
                <div style={{ fontSize: 24 }}>{s.icon}</div>
                <div style={{ fontSize: 32, fontWeight: 900, color: s.color, lineHeight: 1.1 }}>{s.count}<span style={{ fontSize: 12, color: '#aaa', marginLeft: 2 }}>名</span></div>
                <div style={{ fontSize: 11, color: s.color, fontWeight: 700, marginTop: 4 }}>{s.label}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 16, background: '#fffbeb', border: '1.5px solid #fcd34d', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
            <div style={{ fontSize: 24 }}>🍻</div>
            <div style={{ fontSize: 32, fontWeight: 900, color: '#d97706' }}>{partyCount}<span style={{ fontSize: 12, color: '#aaa', marginLeft: 2 }}>名</span></div>
            <div style={{ fontSize: 11, color: '#d97706', fontWeight: 700, marginTop: 4 }}>懇親会参加（合計）</div>
          </div>
          <div style={{ marginTop: 16, background: '#f8fafc', border: '1.5px solid #e2e8f0', borderRadius: 12, padding: '16px', textAlign: 'center' }}>
            <div style={{ fontSize: 13, color: '#6a8090' }}>参加確定 合計</div>
            <div style={{ fontSize: 40, fontWeight: 900, color: '#1a3a2a' }}>{confirmedCount}<span style={{ fontSize: 14, color: '#aaa', marginLeft: 2 }}>名</span></div>
          </div>
        </div>
      </div>
    </div>
...
return (
    <div style={{ fontFamily: "'Hiragino Kaku Gothic ProN','Meiryo',sans-serif", background: '#f0f4f8', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 18px' }}>
        {actionMsg && (
          <div style={{ background: '#f0fdf4', border: '1.5px solid #86efac', borderRadius: 10, padding: '12px 16px', marginBottom: 16, fontSize: 13, fontWeight: 700, color: '#1a3a2a' }}>{actionMsg}</div>
        )}
